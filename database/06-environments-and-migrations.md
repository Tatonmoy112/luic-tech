...
## U03/B004 migration evidence

AUTH-007 adds immutable 0002_catalog after 0001_identity: 13 catalog and three platform reliability tables plus additive generic audit fields/aliases. PostgreSQL's trusted btree_gist extension in catalog provides the variant/currency/channel price interval exclusion. The migrator owns this local database; runtime roles cannot install extensions, approve fixture metadata or change audit/price history. No default broad runtime grants were introduced. [B004](../context/aidlc/bolts/B004-catalog.md) records the exact hash, three-revision fresh rebuild/upgrade/replay, concurrent overlap and role-denial proof. Original local bootstrap audit fields retained identical SHA-256 before/after upgrade.

Runtime readiness now requires all three migration hashes. A B003-only application rejects the expanded history, so rollback requires a compatible application artifact; preserve new catalog/audit/event/outcome facts and repair forward. Historical migration bytes are unchanged. No managed extension approval, zero-downtime rollout, recovery or production acceptance is inferred. Local synthetic compatibility is In review; DB-021 production extension approval remains pending.

## U04/B005 additive migration

AUTH-009 authorizes 0003_stock_cart, SHA-256 3b3870573157727ecf20c6fbdc770f80f163eaabd6adecdf4cc9086f02aca329, five selected inventory/cart tables and restricted guards/grants. Applied to both local/test databases; preserved local upgrade and fresh Linux rebuild passed per [B005](../context/aidlc/bolts/B005-stock-cart.md). Earlier migration bytes and manifests remain immutable. Runtime readiness requires all four hashes; a B004-only application rejects the expanded history. Use compatible code or repair forward without dropping cart/ledger/audit/event facts. This local upgrade runs with demo API stopped; no shared/cloud zero-downtime rollout is claimed. Failure must leave prior history/data intact; verify existing row fingerprints and bootstrap evidence after upgrade.
