...
## B004 catalog refinement, 8 September 2026

AUTH-007 retains the complete B003 pinned package/lock set and runtime config version 2; no package change. PostgreSQL 18.4 btree_gist enforces half-open price intervals in the reviewed local migration. [Catalog contracts](10-local-catalog-contracts.md) fix allowlists, product expected-version ownership, lifetime normalized SKU, exact BDT price strings, bounded filter-bound pagination, current publication eligibility and seven-day command outcome expiry (expired retained keys are not reused). A scoped command lock precedes current staff authorization and product/variant locks. Pending delivery storage does not enable dispatch. [B004](../../context/aidlc/bolts/B004-catalog.md) records 210 passing Linux tests, Windows 204/six skips and final audit zero known vulnerabilities; human and production compatibility acceptance remain pending.

## Approved B005 fixture refinements, 12 September 2026

AUTH-009 approved the B005 revision 1 local plan: fixed inventory.manager role with inventory.read/inventory.adjust; per-command signed adjustment bound of 1,000 units; one TEST-WH; opening fixture through the attributed movement command. Customer and guest carts use the existing 20-unit/50-line bounds and 30-day inactivity duration. Duplicate merge quantities clamp to the stricter SKU/20-unit bound with explicit adjustments; more than 50 distinct lines rejects the entire merge without silently losing items. Terminal source history is retained. These are synthetic technical choices, not real merchant thresholds/retention policy or ADAPT runtime policy administration. [Exact contract](11-local-stock-cart-contracts.md).
