...
const { Database, Telemetry, readConfiguration, FOUNDATION_HASH, IDENTITY_HASH, CATALOG_HASH, STOCK_CART_HASH } = require('../packages/platform/dist');
...
test('reviewed migration identity, replay and concurrent runners agree', async () => {
  ...
  expect(await Promise.all([applyMigrations(connection, folder), applyMigrations(connection, folder)])).toEqual([4, 4]);
  const rows = (await admin.query('SELECT hash FROM drizzle.__drizzle_migrations')).rows;
  expect(rows).toEqual([{ hash: FOUNDATION_HASH }, { hash: IDENTITY_HASH }, { hash: CATALOG_HASH }, { hash: STOCK_CART_HASH }]);
...
test('migration drift is rejected and failed additive migration rolls back without history', async () => {
  ...
  journal.entries.push({ idx: 4, version: '7', when: 1789171200100, tag: '0001_failure', breakpoints: true });
  ...
  expect((await admin.query('SELECT count(*) FROM drizzle.__drizzle_migrations')).rows[0].count).toBe('4');
  // Repair only the never-applied test migration, then prove the lock was released.
  fs.writeFileSync(path.join(temp, '0001_failure.sql'), 'CREATE SCHEMA b002_failed;');
  expect(await applyMigrations(connection, temp)).toBe(5);
  expect(await database.ready()).toBe(false); // future history requires compatible code
  await admin.query('DROP SCHEMA b002_failed');
  await admin.query('DELETE FROM drizzle.__drizzle_migrations WHERE created_at = 1789171200100');
});
